  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      const description = errorDetail
        ? `${data.imported} ingrédient${data.imported > 1 ? 's' : ''} importé${data.imported > 1 ? 's' : ''}, ${data.errors!.length} erreur${data.errors!.length > 1 ? 's' : ''} : ${errorDetail}`
        : `${data.imported} ingrédient${data.imported > 1 ? 's' : ''} importé${data.imported > 1 ? 's' : ''} avec succès`
