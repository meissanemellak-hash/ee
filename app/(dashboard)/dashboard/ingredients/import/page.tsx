...
import { useImportIngredients } from '@/lib/react-query/hooks/use-ingredients'
...
/** Modèle CSV pour l'import des ingrédients (UTF-8 avec BOM, colonnes en français) */
const INGREDIENTS_CSV_TEMPLATE =
  '\uFEFFnom,unite,cout_unitaire,taille_pack,fournisseur\nFarine,kg,0.85,25,Fournisseur A\nBeurre,kg,8.50,5,Fournisseur B\nLait,L,1.20,,'
...
export default function ImportIngredientsPage() {
  ...
  const canImport = permissions.canCreateIngredient(currentRole)
  ...
  const handleDownloadTemplate = useCallback(() => {
    const blob = new Blob([INGREDIENTS_CSV_TEMPLATE], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.setAttribute('download', 'modele_import_ingredients.csv')
    ...
    toast({
      ...
      description: 'Le fichier modele_import_ingredients.csv a été téléchargé. Remplissez-le puis importez-le ci-dessous.',
    })
  }, [toast])
      ...
      <main className="min-h-[calc(100vh-4rem)] bg-muted/25">
        <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
          <header className="pb-6 border-b border-border/60">
            <h1 className="text-3xl font-bold tracking-tight">Import d&apos;ingrédients</h1>
          </header>
          <Card className="rounded-xl border shadow-sm bg-card">
  ...
  if (isLoaded && !organization?.id) {
    return (
      <main className="min-h-[calc(100vh-4rem)] bg-muted/25">
        <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
          <header className="pb-6 border-b border-border/60">
            <h1 className="text-3xl font-bold tracking-tight">Importer des ingrédients</h1>
            <p className="text-muted-foreground mt-1.5">
              Importez vos ingrédients depuis un fichier CSV
            </p>
          </header>
          <Card className="rounded-xl border shadow-sm bg-card">
            <CardContent className="py-16 text-center space-y-4">
              ...
              <p className="text-muted-foreground">
                Veuillez sélectionner une organisation pour importer des ingrédients.
              </p>
              <Button asChild variant="outline">
                <Link href="/dashboard/ingredients">Retour aux ingrédients</Link>
              </Button>
  ...
  return (
    <main className="min-h-[calc(100vh-4rem)] bg-muted/25" aria-label="Importer des ingrédients depuis un fichier CSV">
      <div className="p-6 lg:p-8 space-y-8 max-w-7xl mx-auto">
        <Breadcrumbs items={[{ label: 'Ingrédients', href: '/dashboard/ingredients' }, { label: 'Import' }]} />
          ...
          <Card className="rounded-xl border shadow-sm bg-card">
            <CardHeader>
              ...
              <CardDescription className="mt-1">
                Sélectionnez le fichier CSV à importer. Les ingrédients seront créés dans votre organisation.
              </CardDescription>
            </CardHeader>
                ...
                >
                  ...
                  {file ? (
                    ...
                    <label htmlFor="file-upload" className="cursor-pointer block space-y-2">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Format attendu : nom, unite, cout_unitaire, taille_pack, fournisseur
                      </p>
                    </label>
                  )}
                </div>
          ...
          <CardHeader className="pb-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <CardDescription className="mt-1">
                  Votre fichier CSV doit contenir les colonnes suivantes (en français ci-dessous). Les noms en anglais (name, unit, costPerUnit, packSize, supplierName) sont aussi acceptés. Téléchargez le modèle pour partir d&apos;un fichier prêt à remplir.
                </CardDescription>
              </div>
              <Button
                type="button"
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl bg-muted/30 dark:bg-gray-800/30 border border-border p-4 font-mono text-xs">
                <ul className="space-y-1 list-none p-0 m-0">
                  <li><strong>nom</strong> : Nom de l&apos;ingrédient (obligatoire, unique dans l&apos;organisation)</li>
                  <li><strong>unite</strong> : Unité (kg, L, unité, etc.)</li>
                  <li><strong>cout_unitaire</strong> : Coût par unité en euros (nombre, décimal avec point)</li>
                  <li><strong>taille_pack</strong> : Taille du pack fournisseur (optionnel)</li>
                  <li><strong>fournisseur</strong> : Nom du fournisseur (optionnel)</li>
                </ul>
              </div>
