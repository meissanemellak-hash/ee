...
export default async function DashboardPage(props: PageProps) {
  ...
  let organization: Awaited<ReturnType<typeof import('@/lib/auth').getOrganizationForDashboard>> = null
  try {
    const { getOrganizationForDashboard } = await import('@/lib/auth')
    organization = await getOrganizationForDashboard(userId)
  } catch (err) {
    const { logger } = await import('@/lib/logger')
    logger.error('[Dashboard] getOrganizationForDashboard error:', err)
  }
